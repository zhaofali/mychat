...
public class HttpUtils {
	...
	public static List<ChatMessage> getMessageFromServer(String name)   {
		...
		String respond = "";
		try {
			url = new URL(C.SERVER + "?action=get&name="
			...
			if (respond != "[{}]") {
				...
				List<Message> listMsg = gson.fromJson(respond,
				...
				while (it.hasNext()) {
					...
					Message msg = it.next();
					if (msg.getMessage() == null || msg.getMessage().trim() == "") {
						continue;
					}
			...
			ChatMessage message = new ChatMessage();
			message.setName("宝宝");
			message.setDateStr(new SimpleDateFormat("yyyy-MM-ddHH:mm:ss")
					.format(new Date()));
